...
class NovelServiceImpl(
    ...
    override suspend fun createNovel(novel: UnverifiedNovel): UnverifiedContent {
    ...
    override suspend fun verifyNovel(novelID: Long) {
        ...
        val novel = with(novelData){
            novelRepository.save(Novel(title,japaneseTitle,synopsis,published.toJavaLocalDate(),novelStatus,novelType,demographic,explicitGenre,magazine,null,null,background,finished?.toJavaLocalDate(),chapters,volumes,verifiedContent.id).apply { isNewEntity = true })
        }
