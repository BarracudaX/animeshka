...
class ModerationServiceImpl(private val contentRepository: UnverifiedNewContentRepository) : ModerationService {
    override suspend fun acceptModeration(contentID: Long) {
        ...
        contentRepository.save(content.copy(verifier = verifier, contentStatus = ContentTypeStatus.UNDER_VERIFICATION))
    }
    ...
    override suspend fun rejectContent(contentID: Long, rejectionReason: String) {
        ...
        if(content.contentStatus != ContentTypeStatus.UNDER_VERIFICATION) throw IllegalStateException("Content cannot be rejected because it is not under moderation. Current content status : ${content.contentStatus}.")
